...
export function FloatingInput({ label, className, onChange, onBlur, value, defaultValue, ...props }: FloatingInputProps) {
  const [focused, setFocused] = useState(false);
  const [internalValue, setInternalValue] = useState(defaultValue?.toString() ?? "");
  const hasValue = value !== undefined ? value.toString().length > 0 : internalValue.length > 0;
